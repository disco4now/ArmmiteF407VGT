...
//     }
//
//     dr_int32* pSamples = GetMemory(pFlac->totalSampleCount * sizeof(dr_int32));
...
typedef struct
{
    ...
    // A hack to avoid a GetMemory() when opening a decoder with drflac_open_memory().
    ...
    // Variable length extra data. We attach this to the end of the object so we avoid unnecessary GetMemorys.
...
extern drflac* drflac_open(drflac_read_proc onRead, drflac_seek_proc onSeek, void* pUserData);
...
// Close the decoder with drflac_close().
//
// This is slower than drflac_open(), so avoid this one if you don't need metadata. Internally, this will do a GetMemory()
// and FreeMemory() for every metadata block except for STREAMINFO and PADDING blocks.
//
...
extern void drflac_close(drflac* pFlac);
...
// seeked.
extern dr_uint64 drflac_read_s32(drflac* pFlac, dr_uint64 samplesToRead, dr_int32* pBufferOut);
...
// pointer to the sample data as interleaved signed 32-bit PCM. The returned data must be freed with drflac_FreeMemory().
//
...
void drflac_FreeMemory(void* pSampleDataReturnedByOpenAndDecode);
...
static DRFLAC_INLINE dr_bool32 drflac__reload_l1_cache_from_l2(drflac_bs* bs)
{
    ...
    {
        size_t i, offset = DRFLAC_CACHE_L2_LINE_COUNT(bs) - alignedL1LineCount;
...
{
    int i,channelCount = drflac__get_channel_count_from_channel_assignment(pFlac->currentFrame.header.channelAssignment);
    for (i = 0; i < channelCount; ++i)
    {
...
dr_bool32 drflac__read_and_decode_metadata(drflac* pFlac)
{
    ...
    for (;;)
    {
        ...
        switch (blockType)
        {
            ...
            {
                if (pFlac->onMeta) {
                    void* pRawData = GetMemory(blockSize);
                    if (pRawData == NULL) {
                        return DR_FALSE;
                    }
...
drflac* drflac_open_with_metadata_private(drflac_read_proc onRead, drflac_seek_proc onSeek, drflac_meta_proc onMeta, void* pUserData, void* pUserDataMD)
{
    ...
    drflac* pFlac = (drflac*)GetMemory(allocationSize);
...
dr_uint64 drflac_read_s16(drflac* pFlac, dr_uint64 samplesToRead, dr_int16* pBufferOut)
{
    ...
    dr_uint64 i,totalSamplesRead = 0;
...
{                                                                                                                                                                   \
    ...
    {                                                                                                                                                               \
        ...
        pSampleData = (type*)GetMemory(sampleDataBufferSize);                                                                                                          \
        if (pSampleData == NULL) {                                                                                                                                  \
            goto on_error;                                                                                                                                          \
        }                                                                                                                                                           \
                                                                                                                                                                    \
    ...
    {                                                                                                                                                               \
        ...
        if (dataSize > SIZE_MAX) {                                                                                                                                  \
            goto on_error;  /* The decoded data is too big. */                                                                                                      \
                                                                                                                                                                    ...
                                                                                                                                                                    \
        pSampleData = (type*)GetMemory((size_t)dataSize);    /* <-- Safe cast as per the check above. */                                                               \
        if (pSampleData == NULL) {                                                                                                                                  \
            goto on_error;                                                                                                                                          \
        }                                                                                                                                                           \
                                                                                                                                                                    \
...
void drflac_FreeMemory(void* pSampleDataReturnedByOpenAndDecode)
{
    FreeMemory(pSampleDataReturnedByOpenAndDecode);
}
