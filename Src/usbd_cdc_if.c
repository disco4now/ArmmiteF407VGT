...
extern char BreakKey;                                          // defaults to CTRL-C.  Set to zero to disable the break function
...
volatile USBD_CDC_LineCodingTypeDef LineCoding =
  {
    110, /* baud rate*/
    0x00,   /* stop bits-1*/
    0x00,   /* parity - none*/
...
static int8_t CDC_Init_FS(void)
{
  ...
  hUsbDeviceFS.dev_connection_status=USBD_FAIL;
  return (USBD_OK);
  /* USER CODE END 3 */
}
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
	if(SerialConDisabled){
		...
		int i=0;
		while(i<*Len){
			ConsoleRxBuf[ConsoleRxBufHead]  = Buf[i];   // store the byte in the ring buffer
...
{
  USBD_SetupReqTypedef devret;
  uint8_t result = USBD_OK;
  /* USER CODE BEGIN 7 */
  USBD_StdDevReq(&hUsbDeviceFS, &devret);
  if(hUsbDeviceFS.dev_state != USBD_STATE_CONFIGURED) return USBD_FAIL;
  USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*)hUsbDeviceFS.pClassData;
  if (hcdc->TxState != 0 || LineCoding.bitrate==110){
    return USBD_BUSY;
  }
